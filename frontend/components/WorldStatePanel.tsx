...
const statusText: Record<string, string> = { normal: "正常", suspected_fall: "疑似跌倒", needs_confirmation: "需要确认", safety_concern: "安全关注", outside_safe_zone: "已离开安全区", ready: "待命", executing: "执行中", offline: "离线" };
...
export function WorldStatePanel({ world, visible }: { world: WorldState | null | undefined; visible: boolean }) {
  ...
  return <section className="world-state-panel glass">
    ...
    <div className="world-people">{Object.values(world.people).map((person) => <article key={person.person_id} className={`world-person world-${person.status}`}><span>{person.role === "elder" ? "老人" : "儿童"}</span><b>{person.name}</b><p>{locationText[person.location || ""] || person.location || "位置未知"} · {statusText[person.status] || person.status}</p></article>)}</div>
