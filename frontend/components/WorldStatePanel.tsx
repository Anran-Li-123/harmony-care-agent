...
export function WorldStatePanel({ world, visible }: { world: WorldState | null | undefined; visible: boolean }) {
  if (!world || !visible) return <section className="world-state-panel glass world-waiting"><div className="panel-heading"><div><span className="eyebrow">结果反馈 · 世界状态</span><h2>当前世界状态</h2></div></div><p>运行智能体后，这里展示由上下文与当前事件确定性派生的家庭空间快照。</p></section>;
  return <section className="world-state-panel glass">
    <div className="panel-heading"><div><span className="eyebrow">结果反馈 · 派生状态</span><h2>当前世界状态</h2></div><span className="small muted">非长期存储</span></div>
    ...
    {world.robot && <div className="world-robot"><span>机器人</span><b>{locationText[world.robot.location || ""] || world.robot.location || "位置未知"} · {world.robot.online ? statusText[world.robot.status] || world.robot.status : "离线"}</b></div>}
