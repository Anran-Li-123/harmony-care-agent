...
export function FeedbackSimulationPanel({ goal, enabled, busy, onSubmit }: Props) {
  ...
  return <section className="feedback-simulation-panel glass">
    <div className="panel-heading"><div><span className="eyebrow">第 5 步 · 提交反馈</span><h2>{elderGoal ? "模拟老人反馈" : "模拟监护人反馈"}</h2></div><span className="feedback-waiting">等待中</span></div>
    <p>反馈属于当前看护目标，不会创建新的独立目标。</p>
    <div className="feedback-buttons">
      {elderGoal && <>
        ...
        <button disabled={!enabled || busy} onClick={send({ feedback_type: "watch_activity", source: "elder_watch_01", target_person_id: goal.target_person_id, data: { activity_detected: true } })}>智能手表检测到重新活动</button>
      </>}
      {childGoal && <>
