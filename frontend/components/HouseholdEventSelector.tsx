...
const templates: Record<Target, EventTemplate[]> = {
  elder_li: [
    { id: "elder-fall", label: "跌倒传感器触发", event: { type: "sensor", source: "fall_detector", target_person_id: "elder_li", person: "grandpa", data: { detected: true, location: "bedroom" } } },
    { id: "elder-watch", label: "智能手表长时静止", event: { type: "sensor", source: "watch_activity", target_person_id: "elder_li", person: "grandpa", data: { still_minutes: 45, acknowledged: false, location: "bedroom" } } },
  ...
  child_xiaoyu: [
    { id: "child-door", label: "陌生人敲门", event: { type: "sensor", source: "door_sensor", target_person_id: "child_xiaoyu", person: "child", data: { open: true, visitor: "unknown", guardian_absent: true, location: "entrance" } } },
    { id: "child-zone", label: "离开家庭安全区", event: { type: "sensor", source: "watch_geofence", target_person_id: "child_xiaoyu", person: "child", data: { inside: false, zone: "家庭安全区" } } },
  ...
  household: [
    { id: "family-environment", label: "家庭环境变化", event: { type: "environment", source: "environment_sensor", target_person_id: null, person: null, data: { note: "家庭环境状态发生变化" } } },
    { id: "family-smoke", label: "烟雾传感器触发", event: { type: "environment", source: "smoke_detector", target_person_id: null, person: null, data: { triggered: true, location: "kitchen" } } },
    { id: "family-robot-offline", label: "机器人离线", event: { type: "device", source: "robot", target_person_id: null, person: null, data: { online: false } } },
  ],
};
...
export function HouseholdEventSelector({ context, event, disabled, onChange }: { context: Context; event: CareEvent; disabled: boolean; onChange: (event: CareEvent) => void }) {
  ...
  return <section className="household-event-selector glass">
    <div className="panel-heading"><div><span className="eyebrow">高级步骤 3 · 新事件</span><h2>选择本次新事件</h2></div><span className="small muted">历史与事件相互独立</span></div>
    ...
    <label><span>新事件</span><select value={templateId} disabled={disabled} onChange={(e) => chooseEvent(e.target.value)}>{templates[target].map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label>
