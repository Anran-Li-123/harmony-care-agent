...
const goalStatusText = { planned: "已规划", active: "进行中", awaiting_feedback: "等待反馈", completed: "已完成", cancelled: "已取消" };
...
export function DecisionPanel({ decision, stage }: { decision: Decision | null; stage: number }) {
  ...
  return <section className={`decision-panel glass risk-${decision.risk_level}`}>
    <div className="decision-top"><div><span className="eyebrow">智能体判断 · {decision.llm_mode === "mock" ? "稳定仿真" : "真实模型"}</span><h2>智能体判断</h2></div><div className="risk-badge"><small>风险级别</small><b>{riskText[decision.risk_level]}</b></div></div>
    <p className="decision-summary">{decision.summary}</p>
    {decision.care_goal && <div className="current-goal"><div><span>当前看护目标</span><b>{decision.care_goal.description}</b><small>{decision.care_goal.goal_type}</small></div><strong className={`goal-status status-${decision.care_goal.status}`}>{goalStatusText[decision.care_goal.status]}</strong></div>}
    {decision.goal_evaluation && <div className={`goal-evaluation evaluation-${decision.goal_evaluation.outcome.toLowerCase()}`}><div className="evaluation-head"><span>目标评估</span><b>{decision.goal_evaluation.outcome}</b></div><p>{decision.goal_evaluation.summary}</p><div><strong>风险 {decision.goal_evaluation.previous_risk_level.toUpperCase()} → {decision.goal_evaluation.updated_risk_level.toUpperCase()}</strong><small>目标：{goalStatusText[decision.goal_evaluation.goal_status]}</small></div>{decision.goal_evaluation.evidence.map((item) => <em key={item}>✓ {item}</em>)}</div>}
    {decision.agent_plan && <div className="agent-plan"><div className="agent-plan-title"><b>智能体计划（Agent Plan）</b><span>{decision.agent_plan.summary}</span></div>{decision.agent_plan.steps.map((step, index) => <div className={`plan-step plan-${step.status}`} key={step.step_id}><i>{step.status === "completed" ? "✓" : step.status === "awaiting_feedback" ? "○" : index + 1}</i><div><b>{step.description}</b><small>{step.intent}{step.required_capability ? ` · ${step.required_capability}` : ""}</small></div></div>)}</div>}
