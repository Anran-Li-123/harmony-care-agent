...
export function MemoryEvolution({ beforeContext, context, decision, stage }: { beforeContext: Context; context: Context; decision: Decision | null; stage: number }) {
  ...
  return <section className="memory-section memory-section-v2 glass">
    <div className="panel-heading"><div><span className="eyebrow">MEMORY EVOLUTION</span><h2>记忆与画像如何变化</h2></div><span className="small muted">时间记录不等于长期画像；更新必须有明确或持续证据</span></div>
    <div className="evolution">
      ...
      <article className={stage >= 6 ? "changed" : ""}><small>记忆候选</small><b>{memoryVisible ? `${decision?.memory_updates.length || 0} 条候选` : "等待记忆阶段"}</b><p>{memoryVisible ? decision?.memory_updates.map((item) => item.preview).join("；") || "本次无新增记忆" : "Agent 完成设备行动后再写入记录"}</p></article><i>→</i>
