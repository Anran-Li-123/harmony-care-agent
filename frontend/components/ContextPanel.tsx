...
export function ContextPanel({ context, event, onSave, onUpload }: Props) {
  ...
  return <><section className="family-context-overview glass">
    <div className="panel-heading"><div><span className="eyebrow">家庭上下文（Household Context）</span><h2>{context.household.name}</h2></div><span className="small muted">{people.length} 位家庭成员 · {context.active_history || "兼容场景"}</span></div>
    ...
    {selectedPerson && <div className="person-memory-grid"><article><small>当前画像</small><p>{Object.values(selectedPerson.user_profile).map((item) => `${item.field}: ${String(item.value)}`).join(" · ") || "仅有基础身份，尚未形成长期画像"}</p></article><article><small>工作记忆（Working Memory）</small><p>{selectedPerson.working_memory.slice(-3).map((item) => item.content).join("；") || "暂无工作记忆"}</p></article><article><small>近期事件记忆</small><p>{selectedPerson.episodic_memory.slice(-3).map((item) => item.event).join("；") || "暂无事件记忆"}</p></article></div>}
    ...
    <summary><div><span className="eyebrow">高级数据</span><b>高级导入与原始上下文（Context）</b><small>日常演示无需操作；用于自定义数据和格式检查</small></div><span>展开</span></summary>
    ...
    {editing && <div className="context-modal" role="dialog" aria-modal="true" aria-label="Context JSON 编辑器"><div className="context-modal-card"><header><div><span className="eyebrow">数据结构校验（Schema Validation）</span><b>上下文 JSON 编辑器</b></div><button aria-label="关闭编辑器" onClick={() => setEditing(false)}>×</button></header><textarea value={draft} onChange={(e) => setDraft(e.target.value)} spellCheck={false}/>{error && <p className="error">{error}</p>}<button className="primary wide" onClick={commit}>校验并应用上下文</button></div></div>}
  </details></>;
}
